...
In this update We fully move away from pastebin (and their small file size limit restrictions), and instead move the storage of some code to github. With these changes you will now need to make a [fine-grained github access token](#access_tokens). And you will need to have `curl` and `wget` installed. In regards to evrything else in the code, it's pretty much the same in terms of flow and what not.
...
## Github fine-grained tokens {#access_tokens}
